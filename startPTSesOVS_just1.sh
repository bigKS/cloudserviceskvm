...
TEMPLATE_LOCATION=/home/user/ovsOnly.template.xml
...
for (( X = 1650; X <= 1650; X++)); do
		...
		qcowName=svpts-7.30-cs-1-centos7-amd64-disk1.qcow2c
		...
		cat $TEMPLATE_LOCATION | sed -e "s/PTS_NAME/$vmname/g" \
									 ...
									 -e "s/QCOW_NAME/$qcowName/g" \
									 -e "s/SERVICE_MAC/$macAddress/g" > $domainXml
